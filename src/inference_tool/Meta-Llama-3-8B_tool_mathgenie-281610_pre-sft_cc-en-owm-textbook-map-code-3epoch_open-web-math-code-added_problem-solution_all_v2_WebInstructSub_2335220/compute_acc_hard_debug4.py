    ...
    if gt_asw == asw:
        ...
        try:
            ...
            if str(round(eval(str(latex2sympy(gt_asw))),2)) == str(round(eval(str(latex2sympy(asw))),2)):
                return True
            if is_number(asw) and is_number(gt_asw) and (abs(complex(asw) - complex(gt_asw)) / (abs(complex(gt_asw)) + 1e-30)) <= 0.015 and abs(complex(asw) - complex(gt_asw)) < 1:
                return True
            else:
                return False
        except:
...
def main():
    ...
    print(table)
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "tmp4.md"), "w", encoding="utf-8") as f:
        f.write(table)

if __name__ == "__main__":
    # gt_answer = "300"
    # model_answer = "299.58 \\text{ km/s}"
