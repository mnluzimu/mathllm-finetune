        ...
        try:
            ...
            if "e" in asw and "e" in gt_asw:
                ...
                if is_equal(asw_front, gt_asw_front) and is_equal(asw_end, gt_asw_end):
                    return True
            if is_number(asw) and is_number(gt_asw) and ((abs(complex(asw) - complex(gt_asw)) / (abs(complex(gt_asw)) + 1e-1000)) <= 0.015 and abs(complex(asw) - complex(gt_asw)) < 1):
                return True
...
def main():
    ...
    corrects = []
    totals = []
    # names =  ["ocw", "aime24", "amc23", "aqua", "carp_en", "cn_middle_school", "college_math", "olympiadbench", "sat_math"]
    ...
    print(table)
    with open(os.path.join(os.path.dirname(os.path.realpath(__file__)), "tmp5.md"), "w", encoding="utf-8") as f:
        f.write(table)

if __name__ == "__main__":
    # gt_answer = "4e33"
    # model_answer = "3.95 \\times 10^{33}"
    # print(is_equal(model_answer, gt_answer))
    main()