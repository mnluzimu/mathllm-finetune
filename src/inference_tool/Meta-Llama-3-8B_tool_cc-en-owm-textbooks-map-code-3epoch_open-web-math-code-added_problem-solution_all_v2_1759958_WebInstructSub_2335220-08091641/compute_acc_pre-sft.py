...
def compute_accuracy(in_file, out_file, name):
    ...
    for data in tqdm(datas):
        ...
        last_line = [line for line in data["model_generation"].split("\n")[:-1] if len(line.strip(" \n\t")) > 0][-1]
        model_answer = extract_boxed_answer(data["model_generation"])
        if gt_answer in get_numbers_in_line(last_line):
            answer_in_last_line = True
            model_answer = gt_answer
        else:
            answer_in_last_line = False

        data["answers"] = {"gt_answer": gt_answer, "model_answer": model_answer}
            
        if answer_in_last_line or is_equal(model_answer, gt_answer) or is_equal(model_answer, gt_choice) or ('answer_val' in data['extra'].keys() and is_equal(model_answer, gt_answer)):
...
if __name__ == "__main__":
    ...
    # gt_asw = "1+\\sqrt{3} i"
    # asw = "1 + 1.732i"
    # # gt_asw_exp = str(latex2sympy(gt_asw))
    # # asw_exp = str(latex2sympy(asw))

    # # gt_asw_match = re.fullmatch(r"(.*)([+-])(.*)i", gt_asw)
    # # asw_match = re.fullmatch(r"(.*)([+-])(.*)i", asw)
    # # if is_equal(gt_asw_match.group(1), asw_match.group(1)) and gt_asw_match.group(2) == asw_match.group(2) and is_equal(gt_asw_match.group(3), asw_match.group(3)):
    # #     print("True")
