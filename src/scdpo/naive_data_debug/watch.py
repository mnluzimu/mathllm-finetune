...
def watch():
    ...
    while True:
        for i in range(18):
            file_paths = glob(f"{root_dir}/gsm8k/{i}_round*.jsonl")
            r = sorted([int(file_path.replace(f"{root_dir}/gsm8k/{i}_round", "").replace(".jsonl", "")) for file_path in file_paths])[-1]
            file_path = f"{root_dir}/gsm8k/{i}_round{r}.jsonl"
            if not os.path.exists(file_path):
                length = 0
            else:
                length = len(load_jsonl(file_path))
            source_file = f"{root_dir}/gsm8k/to_be_run_{i}_round{r}.jsonl"
