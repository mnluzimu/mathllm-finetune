...
def restart_session(r, i):
    ...
    cmd = f"tmux new-session -d -s {i} 'python /mnt/cache/luzimu/rlhf_math/src/step_controled_dpo_lce_internlm_no_ch/lce_solution_gen_math.py {r} -i {i} -a 127.0.0.1'"
    os.system(cmd)

def restart_loop(r, i):
    os.system(f"tmux kill-session -t loop{i}")
    cmd = f"tmux new-session -d -s loop{i} 'bash /mnt/cache/luzimu/rlhf_math/src/step_controled_dpo_lce_internlm_no_ch/scripts/math.sh {r} 127.0.0.1 {i}'"
    os.system(cmd)
