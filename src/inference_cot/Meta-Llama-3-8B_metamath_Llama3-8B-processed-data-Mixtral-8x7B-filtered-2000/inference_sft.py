...
def code_generation(query):
    ...
    if result is None:
        messages.append({'role': 'exceed_max_length/return_first_code', 'content': ""})
        jupyter.shutdown()
        return messages

    result = result.replace("<|end_of_text|>", "")
    messages.append({'role': 'text', 'content': result})
...
if __name__ == '__main__':
    ...
    parser.add_argument("i", type=str, help="round number")

    args = parser.parse_args()

    ip = {
        "3epoch_1": "10.119.29.245"
    }
    ...
    for name in ["GSM8K", "MATH", "SVAMP", "mathematics"]:
        ...
        output_path = f'/mnt/cache/luzimu/mathllm-finetune/results/inference_cot/{model_name}/{args.i}/{name}/{name}_result.jsonl'
