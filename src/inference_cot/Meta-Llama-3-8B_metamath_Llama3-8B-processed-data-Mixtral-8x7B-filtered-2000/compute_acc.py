...
def compute_accuracy_ours(in_file, out_file, name):
    ...
    for data in tqdm(datas):
        ...
        model_answer = data["debug_result"][-1]["content"].split("The answer is: ")[-1]
        ...
        if "MATH" in name:
            ...
            if is_equal(model_answer, data["extra"]["answer"]) or ('answer_val' in data['extra'].keys() and is_equal(model_answer, data["extra"]["answer_val"])):
            ...
            if is_equal(model_answer, data["extra"]["answer"]) or ('answer_val' in data['extra'].keys() and is_equal(model_answer, data["extra"]["answer_val"])) or simuleq_equal:
                correct_datas.append(data)
            else:
                wrong_datas.append(data)
...
if __name__ == "__main__":
    ...
    model_name = "Meta-Llama-3-8B_metamath_Llama3-8B-processed-data-Mixtral-8x7B-filtered-2000"

    for name in ["GSM8K", "MATH", "SVAMP", "mathematics"]:
        print(name + ":")
        compute_accuracy_ours(f'/mnt/cache/luzimu/mathllm-finetune/results/inference_cot/{model_name}/{args.i}/{name}/{name}_result.jsonl',
        f'/mnt/cache/luzimu/mathllm-finetune/results/inference_cot/{model_name}/{args.i}/{name}/{name}_result.jsonl',
        name)
